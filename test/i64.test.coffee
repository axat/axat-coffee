...
module.exports =
  Conversions: ->
    ...
    assert.equal i.toString(), '2344560'
    ...
    assert.equal (i = i64.create v; i.toString()), v for v in values

  SomeOperations: ->
    i1 = i64.create()
    i2 = i64.create()
    assert.equal i1.toString(), '0'
    assert.equal i2.toString(), '0'
    i1.low = 17
    assert.equal i1.toString(), '17'
    i2.low = 42
    assert.equal i2.toString(), '42'
    assert.equal i1.add(i1, i2).toString(), '59'
    assert.equal i1.toString(), '59'
    assert.equal i2.toString(), '42'
    assert.equal i1.low, 59
    assert.equal i1.zero().toString(), '0'
    i1.high = 1
    assert.equal i1.toString(), '4294967296'
